## How to generate the synthetic datasets used in the Phase-2 for Q4 that assume public sequences constitute immune signals

- This folder contains all the files required to generate the synthetic datasets used in Phase-2 for Q4, where public sequences are assumed to constitute immune signals. The synthetic datasets are generated using the following console script, assuming this python package is installed:

```bash
nohup multi_simairr -i q1_fullseq_train_config.yaml >> stdout.txt 2>> stderr.txt &
nohup multi_simairr -i q1_fullseq_test_config.yaml >> stdout.txt 2>> stderr.txt &
```
